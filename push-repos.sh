...
for repo in $REPOS; do
  ...
  if grep -rIl 'jlawrence' "$dir" >/dev/null 2>&1 && [ "$GH_USER" != "jlawrence" ]; then
    echo "!! '$dir' still contains the default handle 'jlawrence' but GH_USER=$GH_USER"
    echo "   run: grep -rl jlawrence $dir | xargs sed -i 's|jlawrence|$GH_USER|g'"
  fi
